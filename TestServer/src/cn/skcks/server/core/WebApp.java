...
public class WebApp {
    ...
    static {
        try {
            ...
            // 使用解析器解析
            saxParser.parse(Thread.currentThread().getContextClassLoader().getResourceAsStream("cn/skcks/server/core/WebApp.xml"), servletHandler);
            System.out.println("解析：" + Objects.requireNonNull(Thread.currentThread().getContextClassLoader().getResource("cn/skcks/server/core/WebApp.xml")).getPath());
//
