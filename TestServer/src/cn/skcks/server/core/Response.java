...
//        responseBuilder.append("Content-Type:text/html").append(CRLF);
    ...
    // 动态生成响应内容
    public Response addContent(byte[] content) {
        contentBuilder.append(Arrays.toString(content));
        len += content.length;
        return this;
    }
    ...
    public void responseByte(byte[] data) {
        this.flushContent();
        ...
        try {
            ...
            dataOutputStream.write(String.valueOf(responseBuilder).getBytes(StandardCharsets.UTF_8));
    ...
    public void response(int status) {
        try {
            if (responseBuilder != null) {
                ...
                dataOutputStream.write(String.valueOf(responseBuilder).getBytes(StandardCharsets.UTF_8));
                dataOutputStream.write(String.valueOf(contentBuilder).getBytes(StandardCharsets.UTF_8));
            }
            else{
                responseHeader(500);
                dataOutputStream.writeChars(responseBuilder.toString());
            }
            dataOutputStream.flush();
        } catch (IOException e) {
            e.printStackTrace();
