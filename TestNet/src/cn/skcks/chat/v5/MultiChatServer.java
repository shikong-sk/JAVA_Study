...
//    private static final CopyOnWriteArrayList<Service> clientList = new CopyOnWriteArrayList<>();
    private static final ConcurrentHashMap<String,Service> clientList = new ConcurrentHashMap<>();
...
//                clientList.add(service);    // 管理所有客户端
                ...
                if(clientList.containsKey(service.getUser()))
                {
                    service.send("err@此用户名已被使用");
                    service.release();
                }
                else{
                    clientList.put(service.getUser(),service);
    ...
    static class Service implements Runnable {
        ...
        public void run() {
            login = true;
            getOnlineUser();
            sendToOther("\"" + getUser() + "\" 进入聊天室", true);
            while (run) {
                try {
...
//                            send(string);
        ...
        protected void getOnlineUser() {
            ...
            for (Service otherClient : clientList.values()) {
                if (otherClient == this) {
        ...
        // 群发信息
        private void sendToOther(@NotNull String data, Boolean isSystemMessage) {
            if (data.startsWith("@")) {
                int msgIndex = data.indexOf(":");
                clientList.get(data.substring(1,msgIndex)).send(" [私聊消息] "+ user + " : " +data.substring(msgIndex + 1));
            } else {
                for (Service otherClient : clientList.values()) {
                    if (otherClient == this) {
                        continue;
        ...
        protected void release() {
            ...
            try {
                ...
                if(login)
                {
                    sendToOther("\"" + getUser() + "\" 离开聊天室", true);
                }
                Utils.closeAll(dataOutputStream, dataInputStream, client);
                clientList.remove(getUser(),this);
                run = false;
            } catch (Exception e) {
                e.printStackTrace();
            }
